...
| Styling | scoped Stylus per SFC + shared stable parts | CSS Modules per component + shared stable parts | **in progress**: `@tweeq/styles/shared.styl` owns the button/switch primitives, InputShuffle, and IconIndicator through stable component/part/state attributes; remaining renderer-local rules are tracked by MF-044 |
